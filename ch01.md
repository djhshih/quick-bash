...
expanded into 25719 in the above example, the `ps` program will actually receive `-p` and `25719` as command-line arguments (not `-p` and `$$`).

All core Linux programs come with a **manual** entry, which can be viewed
...
output, which you request using the `--help` or the `-h` argument. (Some
programs such as `ps` treat `-h` as something else, and other programs don't
recognize `--help`. It would be nice if all software programmers follow the
same standards, but some programmers are eccentric.)
...
known as a **script**, which can be run upon request. By convention, shell scripts have the extension `sh`.
...
Then, run the script using `bash` by

```bash
bash hello.sh Brain Fox
```

In the above command, `bash` took `hello.sh`, `Brain`, and `Fox` as arguments and runs the `hello.sh` script with arguments `Brain` and `Fox`. The `hello.sh` script contains a line of command that runs the `echo` program (which simply outputs a given text) with the argument `"Hello, $1."`. This argument is converted by Bash into `Hello, Brain.`, and it is passed to `echo` for output to the terminal.
...
second argument (`Fox`) does not appear in the output. In order to send `Brain
...
The quotes preserves the space characters and protects the argument `Brain Fox` from being split into two arguments.
...
**shebang line** (which beings with `#!` in the script:
...
```bash
/bin/bash hello.sh "Brain Fox"
```

The `.` in `./hello.sh` is necessary so that the shell knows that the
`hello.sh` script is found in the current directory (`.`). The command `/bin/bash` explicitly specifies the path to the `bash` executable (in the event that multiple instances of `bash` are installed on the machine. In our casual use of the script earlier, we only specified `bash` instead of the full path, because executables are installed in standard locations on Linux platforms, and the shell searches standard locations for the first instance of bash.
...
of `./hello.sh` regardless of whether the shebang line is present. Nonetheless, the shebang line also serves as documentation for how the script is meant to be run.


## Parameters

When programs are called, the user may supply values to parameters in order to
control the behaviour of the program. What parameters and how these parameters
are used by the program ultimately depends on the author of the program.
Nevertheless, Linux programs tend to specify parameters in a
similar pattern.
...
**Positional arguments** are generally mandatory and passed arguments are
assigned in order based on position. These required arguments are often
enclosed in angular brackets (inequality signs) `<>`. So, the command

```
program A B
```

will set `pos_arg1` to `A` and `pos_arg1` to `B`.

In contrast, **optional arguments** are *not* mandatory, and they are usually
prefixed by one or two dahses `-` and possibly enclosed by square brackets
`[]`. If the user wishes to specify a value for an optional argument, the user
must also refer to the argument by name. For example,

```
program --opt_arg1 C A B
```

will set `opt_arg1` to `C`, and the positional arguments to the same values as
above. Some programs will require that optional arguments are specified before
position arguments. That is,
...
indicate the end of optional arguments, as in
...
follow. Users should also consult the help usage message or the manual entry
for specific instructions for the program.

It would be helpful to users if you folow these guidelines when you design the
...
parsing, we can use the `getopts` command that is built into Bash.
Alternatively, we can use newer programs such as [`docoptp`][1].

[1]: https://github.com/djhshih/docoptp 
...
together with `c` (known as `C-c`).

The `C-z` combination *suspends* and sends a job into the background (but does
not terminate the job) so that the user can continue to interact with the Bash
command-line and do additional work in parallel to the background job. To
resume the job sent to the background in the background, run `bg`. To resume
a background job by bringing it to the foreground (current command-line
prompt), run `fg`.

If a command is executed with `&` at the end, the job is launched in the background. In other words, the `&` character has a similar effect as pressing `C-z` on a job and resuming it with `bg`.
...
a command to execute: `sleep 1; echo "Woke up."`. This latter command sleeps
for 1 second, and prints `Woke up` to the terminal. After running this command,
the user is returned to the command-line prompt and allowed to issue more
commands. After 1 second, the background running job will complete and print
`Woke up.` to the same terminal. If the `&` were omitted, the user would have
to wait for the command to complete before running additional commands.

The `C-d` combination terminates input to the program, for programs that expect
additional user inputs, such as `cat`. For example, `cat` does not terminal
when the user calls it, because `cat` waits for additioanl input and will
continue to wait unless it recevies the `C-d` signal. Type
...
output file may be incomplete, as the `cat` will be interrupted and terminated
immediately.
...
`$?`          | Exit status of most recently executed foreground program
...
Here, the `author_name` variable is defined and holds the value `Brain Fox`.
...
should print `Brain Fox`.

To recap, `$author_name` is expanded to the value of the `$author_name`
variable (`Brain Fox`) if it is defined in the current Bash session.  The
...
literally, without change, to a new `bash` instance in order to illustrate the
...
a short-hand for `$HOME`). The `~/.bashrc` script is run whenever an interactive `bash` instance is started. If `bash` is started non-interactively (e.g. by using `bash -c`, `bash script.sh`, or `./script.sh`), the `~/.bashrc` script will *not* be run. Therefore, it is important to `export` all variables intended for global use in initialization scripts.

Bash also runs the global `/etc/profile` initialization script as well as the
personal `~/.bash_profile`. These script files are only run in an
interactive, login `bash` instance (e.g. first invoked `bash` inside a terminal
and `bash --login`). As long as the defined variables are exported as
environment variables, the practical differences between define variables in
`~/.bash_profile` and `~/.bashrc` is moot. Additionally, some platforms may
store additional global initializations script `/etc/profile.d/` that are run by
...
`bash` instances (via initialization scripts). The next chapter will elaborate
